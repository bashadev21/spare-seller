...
//    _chatRepository.signInWithToken(currentUser.value.apiToken);
  ...
  createConversation(Conversation _conversation) async {
    ...
    _conversation.users.insert(0, userId);
    _conversation.lastMessageTime =
        DateTime.now().toUtc().millisecondsSinceEpoch;
    _conversation.readByUsers = [userId];
    setState(() {
  ...
  listenForConversations() async {
    ...
    _chatRepository.getUserConversations(userId).then((snapshots) {
      setState(() {
  ...
  listenForChats(Conversation _conversation) async {
    ...
    print(prefs.get('user_id'));
    var userId = (prefs.get('user_id'));
    _conversation.readByUsers.add(userId);
  ...
  addMessage(Conversation _conversation, String text) async {
    ...
    var userId = (prefs.get('user_id'));
    Chat _chat =
        new Chat(text, DateTime.now().toUtc().millisecondsSinceEpoch, userId);
